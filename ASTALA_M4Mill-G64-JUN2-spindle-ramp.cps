...
function onOpen() {
  ...
  writeBlock(gAbsIncModal.format(90), gFeedModeModal.format(94), gFormat.format(91.1), gFormat.format(40), gFormat.format(49), gPlaneModal.format(17) + " G64");
...
function onSection() {
  ...
  if (insertToolCall ||
    ...
    if (!tapping || (tapping && !(properties.useRigidTapping == "without"))) {
      ...
      writeBlock(gFormat.format(4), "P20000"
      );
    }
  }
...
/******THIS SECTION IS FOR TURNING ON THE SPINDLE AFTER 1ST X,Y,Z MOVES******************************************** */
    // if (insertToolCall ||
    //   isFirstSection() ||
    //   (rpmFormat.areDifferent(spindleSpeed, sOutput.getCurrent())) ||
    //   (tool.clockwise != getPreviousSection().getTool().clockwise)) {
